...
class BlogController extends JControllerLegacy{
	...
	//改controller的task，對應controller的function
	public function hello(){	//需要被呼叫才能用
		echo 'this is a task.';
		
	}
	
	public function flower(){	//需要被呼叫才能用
		//echo 'this is a flower.';
		
		//getmodel
		$flower_model=$this->getModel('flowers');
		$flower_data=$flower_model->getflowers();
		
		//getview
		$view=$this->getView('flowers','html');
		$view->flower=$flower_data;	//將$flower_data給$view的flower，所以tmpl的default可以直接用$this->flower
		
		$view->setLayout('happy');	//不讀view/default.php，讀設定的layout檔案：happy
